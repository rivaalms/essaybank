import { type ObjectSchema, type AnyObject, type Maybe, ValidationError, type InferType } from "yup"

type ValidationOpts<IV> = {
   strict: boolean
   abortEarly: boolean
...
type OnValidationSuccess<V> = (values: V) => void
type OnValidationError<E extends Partial<Record<string, string>>, D extends InferType<ObjectSchema<Maybe<AnyObject>>>> = (errors: E, data: D) => void
...
export default function <T extends ObjectSchema<Maybe<AnyObject>>>(
   ...
   async function validate(
      ...
      onError?: OnValidationError<Partial<Record<(keyof InferType<T>), string>>, InferType<T>>
   ): Promise<boolean> {
      let result = false
      ...
      try {
         errors = {}
         await schema.validate(data, { abortEarly: false, ...opts })
            .then((res) => {
               onSuccess(res)
               result = true
            })
